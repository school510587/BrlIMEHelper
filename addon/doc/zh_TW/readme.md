...
點字輸入法助手的狀態包含三個部分：點字緩衝區、點字輸入轉譯模式與輸入法狀態。此為點字輸入法助手狀態訊息的一個範例：
...
使用者在中文輸入模式下，打出來的點字圖案在組字完成之前會先存在點字緩衝區內。例如只有打 145 345 點還不構成字，但是查看點字緩衝區時會提示已經輸入 `⠙⠜` 了。
...
安裝本附加元件之後，點字輸入轉譯模式有兩種，原有的 NVDA 點字輸入與點字輸入法。

NVDA 點字輸入：

點字輸入法：本附加元件接管了點字輸入的處理，將根據下一部分所顯示的輸入法狀態來決定點字輸入的轉譯結果。
...
若有勾選，打字過程中會用第幾點的方式讀出點字緩衝區的每次變化。為了確保此功能順利運作，所有 Unicode 點字字元都應該在目前的語言中有被指定符號名稱或者字元說明（字詞解釋）。
