...
## 簡介
點字輸入法助手 (BrlIMEHelper) 讓使用者可以直接用點字顯示器上的點字鍵盤輸入國字。當沒有點字鍵盤可用，本附加元件也可以把電腦鍵盤模擬成點字鍵盤。使用此附加元件將點字輸入轉換成輸入法的操作，熟練點字規則的使用者不必學習其他輸入法或者鍵盤排列方式，也能打出國字。目前，本附加元件的實作基於台灣廣泛使用的[注音點字](http://class.kh.edu.tw/19061/bulletin/msg_view/75)與微軟注音輸入法，不過未來本附加元件的概念也可以延伸到別的點字系統與輸入法。
...
2. 點字鍵：共九個，分別代表點字 1 至 8 點及點字空白。使用者可以依照輸入轉譯表的規則用它來打字。
...
為了始本節之後的說明簡潔，點字快速鍵就只用點位來描述。把點字快速鍵的點位與其功能聯想在一起，能讓使用者迅速地記住它。例如 "ctrl" 的第一個字母是 c，而英文的 c 是用 1 4 點表示。所以 1 4 點加上點字空白的快速鍵就是按一下 `[Ctrl]` 鍵。有些點字快速鍵用點位的圖形來想像就容易記住。例如 1 2 6 點在英文電腦點字裡代表「小於」，「小於」又像指向左的箭頭。所以 1 2 6 點加上點字空白的點字快速鍵表示按一下方向左 `[`&larr;`]` 鍵。

下面是容易看出來的設計原則：

1. 如果點字快速鍵包含 1 4 點的其中一個，那它的 7 8 點分別表示所模擬的電腦鍵盤快速鍵中分別含有 `[Ctrl]` 及 `[Alt]` 鍵。例如 1 7 點加上點字空白的點字快速鍵代表 `[Ctrl]`(第 7 點)+`[A]`(第 1 點，即英文小寫 a)。
2. 假如點字快速鍵 1 4 點都沒有，那 7 8 點用來區別四個狀況：九宮格數字鍵（沒有 7 8 點）、功能鍵 `[F1]` 到 `[F12]` （有第 7 點）、`[NVDA]` 鍵加上九宮格數字鍵（有第 8 點）、Windows 系統快速鍵（兩個點都有）。
3. 三個 1 到 6 點都沒有的點字快速鍵，是上一條規則的例外情形。

最初設計規劃請見 [nvda-tw 電郵群組的 #3664 號訊息](https://groups.io/g/nvda-tw/message/3664)。

為避免讀起來沒效率，分兩個小節來介紹點字快速鍵。第一個小節中，列出一些常用的點字快速鍵，還有建議讀者如何記住它們。然後第二小節用表列出所有本附加元件制定的所有點字快速鍵。請注意，執行點字輸入法助手功能的點字快速鍵在實體或者虛擬的點字鍵盤上都能使用，但是其他點字快速鍵只能用虛擬點字鍵盤來操作。
...
| 2346  | `[Esc]` 鍵 | &nbsp; |
| 25678 | `[Win]+[D]` 鍵（顯示桌面） | 第 7 8 點 (Win) + 第 1 4 5 點 (d) |
| 678   | `[Win]+[T]` 鍵（跳轉到工具列） | &nbsp; |
| 7     | `[Win]+[Space]` 鍵（在所有語言的所有輸入法之間進行切換） | &nbsp; |
| 78    | `[Win]+[Shift]+[Space]` 鍵（用 `[Win]+[Space]` 的相反順序切換輸入法） | &nbsp; |
