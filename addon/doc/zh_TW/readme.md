...
## 簡介
點字輸入法助手 (BrlIMEHelper) 讓使用者可以直接用點字顯示器上的點字鍵盤輸入國字。當沒有點字鍵盤可用時，本附加元件也能夠把電腦鍵盤（即 QWERTY 鍵盤）模擬成點字鍵盤。使用此附加元件將點字輸入轉換成輸入法的操作，熟練點字規則的使用者不必學習輸入法中的鍵盤對應關係，也能打出國字。目前，本附加元件的實作基於台灣廣泛使用的[注音點字](http://class.kh.edu.tw/19061/bulletin/msg_view/75)與微軟注音輸入法，未來本附加元件的概念也可以延伸到別的點字系統與輸入法。
...
| 鍵盤對應方式 | 標準注音鍵盤 | 標準注音鍵盤 | 標準注音鍵盤（新注音） |
...
以下總表列出使用者必須知道最基本的電腦鍵盤快速鍵。接下來的子章節，對表中的名詞進行必要的解釋。

* NvDA+Ctrl+6: 啟用／停用點字鍵盤模擬功能。
* F, D, S, J, K, L, A, 分號鍵 `[;:]` 與空白鍵：點字鍵盤模擬中的點字第 1 到 8 點與點字空格。
* 0 到 9: 九鍵點字輸入方式中，保留給輸入數字與指定候選字。
...
除使用實體點字鍵盤外，本附加元件提供三種做法幫助使用者透過電腦鍵盤以點字的方式來打字：九鍵點字輸入方式、全鍵盤點字輸入方式和逐點輸入方式。

###### 九鍵點字輸入方式

它就是俗稱的「六點輸入」或「八點輸入」，電腦鍵盤上的九個鍵被拿來模擬實體點字鍵盤，預設為 F, D, S, J, K, L, A, 分號鍵 `[;:]` 與空白鍵。NVDA 處理模擬點字鍵盤所送出的訊息，過程與處理實體點字鍵盤送出的點字輸入訊息一樣。因此，它們讓使用者可以用輸入轉譯表的（點字）規則打字，以及送出點字鍵組成的快速鍵。
...
它適合熟悉電腦鍵盤配置卻不熟輸入法鍵盤對應關係的使用者，跟九鍵點字輸入方式的不同之處在於按鍵與點字圖案間的關係。一個按鍵（可能隨著或不跟隨 Shift）對應到一種點字圖案，這個關係定義在「輸出轉譯表」中。下列兩個例子用來幫助讀者瞭解它的操作和效果。

範例 1:

1. 將輸出轉譯表及輸入轉譯表分別設為「中文 (台灣) 注音」與「Unicode」。
2. 把微軟注音設定成英數輸入轉換模式。
3. 用「切換主鍵盤的輸入方式」快速鍵（見「常用點字快速鍵」）切換到全鍵盤 NVDA 點字輸入。
4. 在編輯區裡打字，就像沒有在模擬什麼點字鍵盤那樣。

四個步驟過後，您會發現編輯區裡的內容摸起來跟剛才打的字一樣，可是實際上它是由 Unicode 點字字元構成，顯示出剛才您輸入點字的過程。也就是說，假如輸入轉譯表不是「Unicode」，這些 Unicode 點字字元會在打出來之前被轉譯回普通的英數符號。

範例 2:

1. 將輸出轉譯表設為「中文 (台灣) 注音」，輸入轉譯表沒有限制。
2. 把微軟注音設定成中文輸入。
3. 用「切換主鍵盤的輸入方式」快速鍵（見「常用點字快速鍵」）切換到全鍵盤點字輸入法。
4. 在編輯區裡打 5 個字元，`a&'=1`，其中 `&` 是 Shift 鍵加主鍵盤的數字 7。

四個步驟之後，您會發現組字區顯示「中文」或者其他也讀ㄓㄨㄥㄨㄣˊ的同音異字。也就是說，「中文」和 `a&'=1` 的點字一樣都是 ⠁⠯⠄⠿⠂。

有些使用者習慣用點字的方式來輸入，但是他們手指關節不夠靈活導致操作實體點字鍵盤或九鍵點字輸入錯誤百出。設計出全鍵盤點字輸入方式即是讓他們的輸入過程變得方便。當觸摸點字時，讀者會經常想到它代表的是哪個字元，而不是它包含第幾點。反過來說，用那些字元來表示想要輸入的點位對使用者而言也一樣簡單。在此輸入方式中，本附加元件根據當下的點字輸出轉譯表將電腦鍵盤上所按的鍵轉譯成點字輸入訊息。例如，目前的點字輸出轉譯表規定把 "f" 轉譯成 ⠋，那麼在電腦鍵盤上按出小寫 f 就代表點字輸入是輸入 ⠋。換句話說，用電腦鍵盤打出 "f" 就等於在點字鍵盤同時按下第 1, 2, 4 點。

就如第一個例子所示，平常的設定之下全鍵盤點字輸入方式在輸入法英數輸入轉換模式的時候看不出作用，因此它又可以當成本附加元件 2.3 版之前版本「在輸入法英數模式中不模擬點字鍵盤」的延伸。

注意：點字快速鍵在此輸入方式中的按法仍遵循九鍵點字輸入方式的規則，用點字空白與其餘點字鍵 1 到 8 點同時按下。

注意：請選擇電腦點字的輸出轉譯表來配合全鍵盤點字輸入方式，否則結果可能不符合預期。例如，標準的一級美語點字將 `ABC` 顯示成 `⠠⠠⠁⠃⠉`，但是 Shift 鍵加 `ABC` 鍵會產生 `⠠⠁⠠⠃⠠⠉` 六個點字圖案，明顯就是不相同。
...
當點字緩衝區內的一串點字圖案足以構成一個字元時，點字組字的程序完成。一般而言，所組成的字元會直接出現在輸入法組字編輯區，可是如果這串點字圖案也是另一個字元的前綴，則被完成的字元在 0.25 秒後才出現。這段時間裡輸入下一個點字圖案會讓點字輸入法助手提早做出決定。假如新的點字圖案能夠追加在緩衝區裡的點字字串上而構成完整字元或字元的前綴，點字組字會繼續而沒有輸出，否則已經組好的字元會被放至輸入法組字編輯區，緩衝區裡只保留新輸入的點字圖案。

例如「∠」的點字圖案 `⠫⠪` 也是「←」的前綴。輸入 `⠫⠪` 後，使用者可以等待 0.25 秒後「∠」出現在輸入法組字編輯區，或者輸入下一個點字圖案。若下一個是 `⠒`，則 `⠫⠪⠒` 構成「←」的前綴而沒有輸出。否則，「∠」被放至輸入法組字編輯區，新輸入的點字圖案卻被緩衝區保留下來。
...
此部分由兩個部件組成，輸入方式與輸入處理器。在例子中，「九鍵」和「點字輸入法」就分別是輸入方式和輸入處理器。

雖然本附加元件提供三種點字輸入方式，但是只有九鍵點字輸入方式和全鍵盤點字輸入方式會被顯示在此，分別用「九鍵」及「全鍵盤」代表。假如沒有啟用點字鍵盤模擬，輸入方式的部件將從缺。

輸入處理器，可以是 NVDA 點字輸入或點字輸入法，一定會顯示出來，它們都是程式裡收集與解釋點字輸入的一部分。本來的 NVDA 點字輸入處理器，以 NVDA 點字輸入來表示，將收集到的點字輸入透過輸入轉譯表轉成意思一樣的鍵盤訊息，並將其送出給系統。而「點字輸入法」則表示本附加元件接管了點字輸入的轉譯。
...
| 點位（加上點字空白） | 功能 | 速記方法 |
| :------------------- | :--- | :------- |
| 456 | 在輸入法的中文輸入及英數輸入轉換模式之間切換 | 與「視窗導盲鼠系統」用相同點位 |
| 245 | 清空點字緩衝區以重新輸入正確內容 | 錯（ㄘㄨㄛˋ）的ㄘ是第 2 4 5 點 |
| 123 | 切換主鍵盤的輸入方式 | 與「視窗導盲鼠系統」用相同點位 |
...
| 123   | 切換主鍵盤的輸入方式 | `[Ctrl]+[L]` | `[Alt]+[L]` | `[Ctrl]+[Alt]+[L]` |
...
| 456   | 在輸入法的中文輸入及英數輸入轉換模式之間切換 | 無 | 無 | 無 |
...
有時候，點字輸入法助手沒有在 NVDA 啟動時成功從註冊表載入所有輸入法的資料。這造成它以為此電腦尚未安裝所支援的中文輸入法，而沒有提供中文點字輸入的功能。解決這個問題，可以用本項目要求點字輸入法助手重新載入輸入法的資料，而不需要讓 NVDA 重新載入所有附加元件。
...
#### 預設以全鍵盤輸入方式來操作 NVDA 點字輸入

若有勾選，預設就是用全鍵盤輸入方式配合 NVDA 點字輸入。換句話說，「全鍵盤 NVDA 點字輸入」是預設的輸入模式。

註：此選項延伸自 2.3 版之前的「在輸入法英數模式中預設不模擬點字鍵盤」。在 2.7 版之前則稱為「在輸入法英數輸入中模擬點字鍵盤時預設為 ASCII 模式」。
...
使用者必須讓這個選項跟輸入法中文輸入轉換模式中的鍵盤對應方式維持一致，以確保本附加元件正常運作。注意目前不支援使用者自訂的鍵盤對應方式。
...
- 目前已知的程式錯誤：
    ...
    * 用微軟的日文輸入法和全鍵盤輸入方式時，放開大寫鎖定鍵的訊息從此序列中消失：按下大寫鎖定鍵、按下 Shift 鍵、放開大寫鎖定鍵、放開 Shift 鍵。
    * [Braille Extender](https://github.com/aaclause/brailleExtender) 目前與本附加元件不相容。
- 未來可能的改進：
    * 允許使用者自訂本附加元見的預設狀態。
...
### 2.7 版
* 支援 NVDA 2025.1 版。
* 更名／改寫點字輸入的方式：「九鍵」與「全鍵盤」輸入方式。
* 即使附加元件載入時沒有找到認得的注音輸入法，仍保留九鍵 NVDA 點字輸入的功能。
* 將輸入模式的資訊加入輸入轉換模式改變的訊息中。
* 解決安全桌面中無法查詢鍵盤排列的問題。
* 其他程式碼重構、說明文件的調整。
...
* 支援 NVDA 2026.1 版。
* 加入「重新載入輸入法資料」功能表項目。
* 修復點字輸入法助手無法與 Windows 11 上的記事本和主控台應用程式配合使用的錯誤。
